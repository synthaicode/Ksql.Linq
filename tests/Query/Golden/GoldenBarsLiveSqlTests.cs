...
public class GoldenBarsLiveSqlTests
{
    [KsqlTopic("rate")]
    private class Rate
    {
    ...
    private static KsqlQueryModel BuildModel()
    {
        return new KsqlQueryRoot()
            .From<Rate>()
            .Tumbling(r => r.Timestamp, new Windows { Minutes = new[] { 5 } })
    ...
    [Fact]
    public void Bars5mLive_Equals_Golden()
    {
        ...
        GoldenSqlHelpers.AssertEqualsOrUpdate(
            goldenPath: "tests/Query/Golden/bars_5m_live.sql",
            actualSql: sql);
    }
}
