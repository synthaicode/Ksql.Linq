...
| 20 | `.Where(e => e.Items.Select(i => i.Price).Sum() > 100)` (SELECT + 集約) | `InvalidOperationException: Aggregate functions are not allowed in WHERE clause.` | `.SelectMany(e => e.Items, (e, i) => new { e.Id, i.Price }).GroupBy(x => x.Id).Where(g => g.Sum(x => x.Price) > 100)` |
